...
from common.misc_util import adjust_lr, get_n_params, visualize_attn
import torch
...
class PPO(BaseAgent):
    ...
    def evaluate(self, obs, hidden_state, done):
        with torch.no_grad():
            obs = torch.FloatTensor(obs).to(device=self.device)
            hidden_state = torch.FloatTensor(hidden_state).to(device=self.device)
            mask = torch.FloatTensor(1-done).to(device=self.device)
            dist, value, hidden_state, c1, c2, c3 = self.policy.evaluate(obs, hidden_state, mask)
            act = dist.sample()
            log_prob_act = dist.log_prob(act)

        return act.cpu().numpy(), log_prob_act.cpu().numpy(), value.cpu().numpy(), hidden_state.cpu().numpy(), c1, c2, c3
    ...
    def test(self, num_timesteps, chk_pth):
        # load learned model
        chk = torch.load(chk_pth)
        self.policy.load_state_dict(chk['state_dict'])
        # evaluate policy
        obs = self.env.reset()
        hidden_state = np.zeros((self.n_envs, self.storage.hidden_state_size))
        done = np.zeros(self.n_envs)
        ...
        while self.t < num_timesteps:
            self.policy.eval()
            for _ in range(self.n_steps):
                act, log_prob_act, value, next_hidden_state, c1, c2, c3 = self.evaluate(obs, hidden_state, done)
                next_obs, rew, done, info = self.env.step(act)
                self.storage.store(obs, hidden_state, act, rew, done, info, log_prob_act, value)
                # save attention combined images for review
                vis_maps = visualize_attn(obs, c1, c2, c3)
                step += 1
                self.logger.add_image(vis_maps, step)
                obs = next_obs
                hidden_state = next_hidden_state
